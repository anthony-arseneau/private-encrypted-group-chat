...
/**
 * Class that helps keeping a whitelist of authorized users in the group chat
 * 
 * Responsabilities:
...
public class Whitelist {
    ...
    // Constructor
    public Whitelist() throws NoSuchAlgorithmException {
    ...
    public void addToWhitelist() throws IOException {
        ...
        while (looping) {
            ...
            if (input.equals("done")) looping = false; // Done adding users
                ...
                byte[] usernameHashed = digest.digest(newUsername.getBytes(StandardCharsets.UTF_8));
                String usernameHashedHex = Converter.bytesToHex(usernameHashed);

                // Hash password and get hex string value
                byte[] passwordHashed = digest.digest(newPassword.getBytes(StandardCharsets.UTF_8));
                String passwordHashedHex = Converter.bytesToHex(passwordHashed);

                // Save credentials
                addUser(usernameHashedHex, passwordHashedHex);
            }
        }
        reader.close();
